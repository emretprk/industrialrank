...
# bestseller_urls.txt is pre-validated by scrape_bestsellers.py, so we only
# need the categorize() filter here (the old 4+ digit regex would wrongly
# reject some valid SKUs like "...-a.html" or "...-bndl.html").
urls = [u for u in all_urls if categorize(u)]
